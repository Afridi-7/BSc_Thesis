...
```
                    +---------------------------------------------+
                    ï¿½                  config.yaml                ï¿½
                    ï¿½       (single source of truth, YAML)        ï¿½
                    +---------------------------------------------+
                                           ï¿½
                                           ?
+--------------+   +--------------+   +--------------+   +--------------------+
ï¿½   Image      ï¿½--?ï¿½  Stage 1     ï¿½--?ï¿½  Stage 2     ï¿½--?ï¿½  Stage 3           ï¿½
ï¿½  (.jpg/.png) ï¿½   ï¿½  YOLOv8      ï¿½   ï¿½ EfficientNet ï¿½   ï¿½  RAG + LLM Agent   ï¿½
+--------------+   ï¿½  detector    ï¿½   ï¿½  + MC-Dropoutï¿½   ï¿½  (LangChain ReAct) ï¿½
                   +--------------+   ï¿½  + Grad-CAM  ï¿½   +--------------------+
                                      +--------------+            ï¿½
                                                                  ?
                                                       +----------------------+
                                                       ï¿½ ChromaDB vector storeï¿½
                                                       ï¿½ (1,049 PDF chunks,   ï¿½
                                                       ï¿½  MiniLM embeddings)  ï¿½
                                                       +----------------------+

                                  +------------------------------------+
                                  ï¿½  Two consumers of src.pipeline:    ï¿½
                                  ï¿½  ï¿½ main.py             (CLI)       ï¿½
                                  ï¿½  ï¿½ backend/main.py     (FastAPI)   ï¿½
                                  +------------------------------------+
                                                  ï¿½
                                                  ?
                                       +------------------+
...
**Key property:** [src/pipeline.py](src/pipeline.py) is the single orchestrator. The CLI and the HTTP server both import the same `BloodSmearPipeline` class ï¿½ there is no duplicated inference code, no double model load.

---

## Models and data

### Stage 1 ï¿½ Detection (YOLOv8s)
...
### Stage 2 ï¿½ Classification (EfficientNet-B0)

- **Checkpoint**: [models/efficientnet_wbc_finetuned.pt](models/efficientnet_wbc_finetuned.pt) (~16 MB)
- **Architecture**: `timm/efficientnet_b0` with a custom 8-class head, fine-tuned on the **PBC dataset** (Acevedo *et al.*, Mendeley Data ï¿½ peripheral blood cells).
...
- **Uncertainty quantification**: **Monte Carlo Dropout** with 20 stochastic forward passes per cell. Produces:
  - `entropy` ï¿½ predictive uncertainty across passes
  - `margin` ï¿½ gap between top-1 and top-2 class probability
  - `bucket` ? {`low`, `medium`, `high`} per the thresholds in `config.yaml` ? `classification.uncertainty`
- **Interpretability**: **Grad-CAM** on the last MBConv block ï¿½ see [src/classification/](src/classification/). Each WBC gets a heatmap overlay so reviewers can verify the model attended to the cell, not artefacts.

### Stage 3 ï¿½ Reasoning (RAG + GPT-4o)
...
- **Agentic mode** (`reasoning.mode: agent`): a **LangChain ReAct agent** (`langgraph.prebuilt.create_react_agent`) with five tools:
  1. `query_knowledge_base` ï¿½ semantic search over the textbook corpus
  2. `lookup_lab_reference_ranges` ï¿½ normal value tables for CBC analytes
  3. `interpret_differential` ï¿½ heuristics over WBC class proportions
  4. `get_uncertainty_summary` ï¿½ surfaces high-uncertainty cells
  5. `get_detection_counts` ï¿½ exposes Stage 1 totals to the agent
...
Inference does **not** require the training datasets ï¿½ the checkpoints under `models/` are self-contained.
...
ï¿½
+-- src/                    # Core inference library ï¿½ importable as `src.*`
ï¿½   +-- pipeline.py         # End-to-end orchestrator (Stage 1 ? 2 ? 3)
ï¿½   +-- config/             # YAML + .env loader, validation
ï¿½   +-- detection/          # YOLOv8 wrapper
ï¿½   +-- classification/     # EfficientNet, MC-Dropout, Grad-CAM
ï¿½   +-- rag/                # PDF processor, retriever, reasoner, ReAct agent
ï¿½   +-- utils/              # Logging, validators, metrics, helpers
ï¿½
+-- backend/                # FastAPI HTTP layer over src.pipeline
ï¿½   +-- main.py             # `app` factory + uvicorn entry
ï¿½   +-- version.py          # __version__ string
ï¿½   +-- config.py           # Web-only settings (CORS, upload limits)
ï¿½   +-- dependencies.py     # Lazy pipeline singleton
ï¿½   +-- schemas.py          # Pydantic request/response DTOs
ï¿½   +-- routes/             # /api/health, /api/samples, /api/analyze
ï¿½   +-- services/           # Bounding-box overlay renderer
ï¿½
+-- frontend/               # Vite + React + TypeScript SPA
ï¿½   +-- package.json
ï¿½   +-- vite.config.ts      # Proxies /api ? http://localhost:8767
ï¿½   +-- src/
ï¿½       +-- api.ts          # Typed fetch client
ï¿½       +-- types.ts        # Mirrors backend Pydantic schemas
ï¿½       +-- components/     # Header, UploadCard, DetectionPanel,
ï¿½       ï¿½                   # ClassificationPanel, GradCamPanel,
ï¿½       ï¿½                   # ReasoningPanel, AgentTracePanel
ï¿½       +-- App.tsx
ï¿½
+-- models/                 # Trained .pt checkpoints (gitignored ï¿½ too large)
+-- data/
ï¿½   +-- pdfs/               # Stage-3 hematology textbooks
ï¿½   +-- chroma_db/          # Persisted vector store (built on first run)
...
A separate `backend/.env` or `backend/requirements.txt` would mean the CLI and tests can't find the API key or the dependencies. **One project, one venv, one `.env`, one `requirements.txt`** ï¿½ and `backend/` contains *only* HTTP-layer code.
...
| OpenAI API key | ï¿½ | Stage 3 reasoning (GPT-4o) |
| Internet | ï¿½ | First-time `pip install`, OpenAI API calls; optional Stage-3 web augmentation |
| Disk | ~3 GB free | venv, models, vector store |
| RAM | 8 GB | CPU inference is comfortable; 16 GB recommended for parallel batch runs |
| GPU | Optional | CUDA accelerates Stages 1 + 2 by ~10ï¿½ |
...
The first Stage 3 invocation builds the ChromaDB index (~60 s on CPU). Subsequent runs reuse the persisted store ï¿½ typical end-to-end latency on a sample image is **8ï¿½12 s**.

---

## Daily workflow

### TL;DR ï¿½ one-click launcher
...
That single script kills any stale processes on ports 8767 and 5173, starts the backend in one new PowerShell window, the frontend in another, waits for the backend health check to pass, and opens your browser on http://localhost:5173. **This is the recommended way to run the app for daily use and demos** ï¿½ no more "backend unreachable" surprises.
...
### Terminal 1 ï¿½ backend (FastAPI on :8767)
...
### Terminal 2 ï¿½ frontend (Vite on :5173)
...
- **Frontend terminals don't need the venv** ï¿½ `npm` is Node, not Python.
...
> PowerShell wildcards behave inconsistently ï¿½ pass explicit paths if a glob like `images\*.jpg` doesn't expand.
...
*Figure A ï¿½ Initial UI: drop zone, bundled samples, and empty stage cards.*

![Stage 1 detection panel with YOLOv8 bounding-box overlay.](figures/webapp/01_detection.png)

*Figure B ï¿½ Stage 1: YOLOv8 detection. WBC/RBC/Platelet counts and an annotated overlay with per-box confidences.*
...
*Figure C ï¿½ Stage 2: per-cell WBC class distribution, MC-Dropout uncertainty (mean confidence / entropy / variance / flagged-for-review fraction), and Grad-CAM saliency for each cropped WBC.*

![Stage 3 RAG-grounded clinical reasoning narrative with citations and safety flags.](figures/webapp/03_reasoning.png)

*Figure D ï¿½ Stage 3: GPT-4o agent narrative with key findings, differential diagnoses tagged with citations, recommendations, and safety flags.*

![Agent trace showing the ReAct tool-call sequence.](figures/webapp/04_agent_trace.png)

*Figure E ï¿½ Agent trace: each step shows which tool the LLM invoked (e.g. `lookup_lab_reference_ranges`, `query_knowledge_base`, `get_uncertainty_summary`) and the observation it received back. This is the system's explainability surface.*

![Raw JSON response (debug view).](figures/webapp/05_raw_response.png)

*Figure F ï¿½ Raw response: the full structured JSON returned by `POST /api/analyze`, including metadata, per-stage outputs, and timing ï¿½ useful for debugging and reproducibility audits.*
...
1. **Detection** ï¿½ annotated overlay (boxes color-coded per class) + per-class count chips.
2. **Classification** ï¿½ bar chart of WBC class proportions, per-cell rows with confidence and uncertainty bucket.
3. **Grad-CAM Saliency** ï¿½ thumbnail grid of heatmap-overlaid WBC crops (interpretability check).
4. **Reasoning** ï¿½ final markdown answer + grounded citations + safety flags + `requires_expert_review` badge.
5. **Agent Trace** ï¿½ collapsible list of ReAct steps (Thought / Tool call / Observation / Final). Only populated in `reasoning.mode: agent`.
...
**Use a different OpenAI-compatible endpoint** ï¿½ set `OPENAI_BASE_URL` in `.env` and change `llm.model_name` (works with Azure OpenAI, vLLM, Groq's OpenAI shim, etc.).
...
Stage 3 talks an **OpenAI-compatible REST API**, so swapping models is a config change ï¿½ no Python edits required.
...
| **RunPod / Vast.ai vLLM pod** | ~$0.20ï¿½0.40 / hr (T4/A10) | 5 min | Best price for ad-hoc demos. Pause when done. |
...
### Defence-day checklist (HF Inference Endpoints ï¿½ recommended)
...
7. Edit `.env` at the repo root ï¿½ **add these two lines** (keep your existing `OPENAI_API_KEY` line as a fallback, but the values below override it for this endpoint):
   ```env
...
Just undo step 7 and 8 ï¿½ comment out `OPENAI_BASE_URL`, restore your real `OPENAI_API_KEY=sk-...`, and set `llm.model_name: "gpt-4o"`. No restart issues; the same code path handles both.
...
- If the kernel won't release it (rare), reboot ï¿½ or just use another port: `uvicorn main:app --reload --port 8002` and set `$env:VITE_BACKEND_URL = "http://localhost:8002"` before `npm run dev`.
...
- Vite dev server caches its proxy config ï¿½ restart `npm run dev` after editing [frontend/vite.config.ts](frontend/vite.config.ts).

### Stage 3 takes ~60 s on first run

That's the one-time ChromaDB index build over the two PDFs (1,049 chunks ï¿½ MiniLM encoding). Subsequent runs reuse `data/chroma_db/` and reasoning is ~2ï¿½8 s.

### Grad-CAM panel is empty

- `classification.gradcam.enabled: false` in `config.yaml` ï¿½ flip it to `true`.
- Stage 1 detected zero WBCs ï¿½ check the *Detection* card.

### Agent trace is empty

- `reasoning.mode: linear` in `config.yaml` ï¿½ set to `agent`.
...
It's a Bayesian approximation that requires **no architectural change** ï¿½ just keeping dropout active at inference and averaging over N stochastic forward passes. It captures epistemic uncertainty (model "doesn't know") which is the kind we care about for safety flagging.

**3. Why an agent instead of a single LLM call?**
A single-shot RAG call answers *what* but rarely *why*, and never asks for clarifying evidence. The ReAct agent decomposes the problem: it can call `lookup_lab_reference_ranges` for a numeric anchor, *then* `query_knowledge_base` for context, *then* `interpret_differential` to sanity-check class proportions. The trace is rendered to the user ï¿½ **explainability comes for free**.
...
Separation of concerns. `src/` contains domain logic and is testable / Jupyter-importable without spinning up a web server. `backend/` is a thin HTTP adapter ï¿½ schemas, routes, DI. Either layer can be replaced (e.g. swap FastAPI for gRPC) without touching the other.
...
Higher quality than open-weight alternatives on clinical reasoning benchmarks at the time of writing, and the JSON-mode + function-calling support is rock-solid for the agent's tool-use loop. The system is **provider-agnostic** at the config level ï¿½ switch `llm.model_name` and set `OPENAI_BASE_URL` to use any OpenAI-compatible endpoint.
...
- **Class imbalance.** The `erythroblast` and `basophil` classes are minority in the training data ï¿½ predictions on these are correspondingly less reliable, which is reflected in the uncertainty scores.
...
- **OpenAI data policy.** Image content used in Stage 3 prompts (textual descriptors only ï¿½ actual pixels never leave the local machine) is subject to OpenAI's API data policy. Disable Stage 3 for sensitive workloads.
