...
7. **Agentic reasoning with GPT-4o and LangGraph ReAct.** The main reported Stage 3 configuration uses GPT-4o as the hosted reasoning backend inside a tool-using ReAct agent with knowledge-base search, laboratory reference ranges, differential interpretation, uncertainty summaries, and detection-count access. The resulting tool trace is preserved as an audit artefact.
8. **A domain-specialised LLM artefact.** To justify and reproduce the "domain-expert" component beyond hosted GPT-4o prompting, the project prepared and released a Llama-3.1-Instruct 8B QLoRA/LoRA adapter trained on 2,098 grounded hematology Q&A pairs synthesised from the project's 1,049 ChromaDB textbook chunks. The adapter is hosted at <https://huggingface.co/Afridi07/hematology-llama-3.1-8b-lora> and can be served through an OpenAI-compatible endpoint, such as vLLM, then selected through `OPENAI_BASE_URL` and `config.yaml`. It is a reproducible self-hostable artefact, not a clinically validated replacement for the default GPT-4o backend.
9. **Deterministic expert-review flags.** The `requires_expert_review` flag is computed by policy logic over uncertainty and safety signals rather than delegated to the LLM, making the safety pathway reproducible and inspectable.
10. **A reproducible and auditable engineering architecture.** The project is released as a configuration-driven Python library, FastAPI backend, React+TypeScript frontend, and pytest-covered supplementary artefact, enabling the reported experiments and demonstrations to be inspected and rerun.
...
The default reasoning backend used for the main reported Stage 3 experiments is OpenAI's GPT-4o, accessed via the official OpenAI Python SDK at temperature 0.1. The choice of GPT-4o for the primary experiments is motivated by:

- strong general clinical-reasoning performance at the time of writing,
- strong, reliable structured-output (JSON-mode and function-calling) support that is critical for the agent's tool-use loop,
- low setup burden for a reproducible thesis demonstration, because no local GPU is required,
- low API latency relative to many self-hosted alternatives at comparable quality.

The system is **provider-agnostic** at the configuration level: a different `llm.model_name` and an `OPENAI_BASE_URL` environment variable will route requests to any OpenAI-compatible endpoint (Azure OpenAI, vLLM, Groq's OpenAI shim, etc.). Stage 3 has been smoke-tested against `gpt-4o-mini` for cost-sensitive runs, and the repository also includes a domain-specialised open-weight adapter described below. Thus, GPT-4o should be read as the evaluated default backend, not as a hard architectural requirement.

#### 6.5.1 Domain-specialised self-hostable reasoner

The phrase "Domain-Expert LLM Reasoning" in the thesis title refers to two complementary components. First, the main runtime reasoner is domain-constrained through RAG over hematology textbooks, domain-specific tools, deterministic safety rules, and an auditable ReAct trace. Second, the project also includes a self-hostable domain-specialised language-model artefact that can replace the hosted backend in future deployments.

This artefact is a **Llama-3.1-Instruct 8B** model adapted with **QLoRA / LoRA**. The fine-tuning data consist of **2,098 grounded hematology Q&A pairs** synthesised from the same **1,049 ChromaDB textbook chunks** used by the RAG system. The intended pipeline is:

```
textbook chunks -> grounded Q&A generation -> QLoRA fine-tuning of Llama-3.1-Instruct 8B -> LoRA adapter export -> optional vLLM/OpenAI-compatible serving
```

The adapter is hosted at <https://huggingface.co/Afridi07/hematology-llama-3.1-8b-lora>. The corresponding reproducibility files are [Notebooks/Domain_Expert_Finetune/domain_expert_finetune.ipynb](Notebooks/Domain_Expert_Finetune/domain_expert_finetune.ipynb), [scripts/build_finetune_dataset.py](scripts/build_finetune_dataset.py), and [scripts/evaluate_finetuned_model.py](scripts/evaluate_finetuned_model.py). Because Stage 3 communicates with OpenAI-compatible endpoints, this adapter can be served with a tool such as vLLM and selected through `OPENAI_BASE_URL` and `config.yaml` without changing the rest of the pipeline.

The model was trained to reduce dependence on a commercial API, support future on-premise deployment, and make the domain-specialised component reproducible. It is not presented as the default backend for the reported Stage 3 results, because local serving of an 8B model requires GPU resources and would add cost and operational complexity to ordinary experiments and demonstrations. A full quantitative comparison against GPT-4o and GPT-4o-mini on faithfulness, citation support, clinical correctness, latency, and cost is left as future work.
...
- **OpenAI dependency.** The main reported Stage 3 experiments use GPT-4o through a hosted commercial API. This limitation is partly mitigated by the prepared Llama-3.1-Instruct 8B LoRA adapter, which provides a self-hostable domain-specialised alternative through an OpenAI-compatible endpoint. However, local serving requires suitable GPU resources, and a formal comparative evaluation against GPT-4o was not completed; therefore the hosted GPT-4o backend remains the main evaluated configuration.
...
3. **Domain-specialised reasoner evaluation.** The domain-specialised Llama-3.1-Instruct 8B LoRA adapter has been built and released, but it has not yet been formally compared against the hosted GPT-4o backend used in the main Stage 3 experiments. The remaining work is a controlled comparison between GPT-4o, GPT-4o-mini, and the fine-tuned Llama LoRA on faithfulness, citation support, clinical correctness, latency, and cost. Future work should also test practical local serving through vLLM and, where feasible, llama.cpp-style quantised deployment to determine whether the self-hosted route is realistic for low-resource or privacy-sensitive settings.
